...
/** Hitung percobaan gagal dalam jendela waktu (fail-open bila tabel belum ada). */
async function countRecentFails(
  ...
  try {
    ...
    // Fail-open bila migrasi belum dijalankan — login tetap berfungsi.
    console.error('[customer-login] Gagal cek rate limit:', e)
    return { byWhatsapp: 0, byIp: 0 }
  }
}
...
async function recordFailure(admin: SupabaseAdmin, whatsapp: string, ip: string): Promise<void> {
  try {
    ...
    console.error('[customer-login] Gagal mencatat percobaan:', e)
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    // --- RATE LIMITING: blokir brute force PIN ---
    const ip = getClientIp(req)
    const { byWhatsapp, byIp } = await countRecentFails(admin, normalized, ip)
