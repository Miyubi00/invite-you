...
const AbstractCanvas = ({  type, groom = '', bride = ''  }: { type: string; groom?: string; bride?: string })  => {
  switch (type) {
    case 'cover':
      return (
...
const RsvpContent = ({  onRsvpSubmit, submittedData, rsvps  }: { onRsvpSubmit?: (payload: RsvpPayload) => Promise<void>; submittedData?: RsvpRow | null; rsvps: RsvpRow[] })  => {
    ...
    <div className="max-w-3xl mx-auto grid md:grid-cols-2 gap-8 animate-in fade-in duration-500">
        ...
        {submittedData ? (
          <div className="text-center py-10">
            ...
            <div className="mt-4 bg-white p-3 rounded text-left text-sm border border-gray-200">
              ...
              <p className="mb-2">{submittedData.status === 'hadir' ? 'Hadir' : submittedData.status === 'ragu' ? 'Masih Ragu' : 'Tidak Hadir'}</p>
      ...
      <div className="bg-white p-6 rounded-lg border border-gray-200 max-h-[400px] overflow-y-auto custom-scroll">
        ...
        <div className="space-y-2">
          {rsvps?.length > 0 ? rsvps.map((rsvp: RsvpRow) => (
...
const ContentRenderer = ({  id, groom, bride, date, guestName, data, timeLeft, onRsvpSubmit, submittedData  }: { id: string; groom: string; bride: string; date: string; guestName: string; data?: TemplateData; timeLeft: TimeLeft; onRsvpSubmit?: (payload: RsvpPayload) => Promise<void>; submittedData?: RsvpRow | null })  => {
  ...
  switch (id) {
        ...
        <div className="grid md:grid-cols-2 gap-8 items-center animate-in fade-in slide-in-from-bottom duration-500">
          <div className="text-center space-y-4">
            <div className="w-48 h-64 mx-auto bg-gray-200 border-8 border-white shadow-lg rotate-[-2deg]">
              <img src={data?.groom_photo || DEFAULT_PHOTOS.groom} alt="Groom" className="w-full h-full object-cover" />
            </div>
          ...
          <div className="text-center space-y-4">
            <div className="w-48 h-64 mx-auto bg-gray-200 border-8 border-white shadow-lg rotate-[2deg]">
              <img src={data?.bride_photo || DEFAULT_PHOTOS.bride} alt="Bride" className="w-full h-full object-cover" />
            </div>
    ...
    case 'event': {
      ...
      const fallbackDate = new Date(date).toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom duration-500 max-w-2xl mx-auto">
          ...
          <div className="grid gap-6">
            <div className="bg-white p-6 border-l-4 border-[#2F4F4F] shadow-sm">
              ...
              <p className="text-lg text-gray-700">{formattedAkadDate}</p>
              <p className="text-2xl font-light text-gray-800">{schedule.akadTime}</p>
            </div>
            ...
            <div className="bg-white p-6 border-l-4 border-[#8B7355] shadow-sm">
              ...
              <p className="text-lg text-gray-700">{formattedResepsiDate}</p>
              <p className="text-2xl font-light text-gray-800">{schedule.resepsiTime}</p>
            </div>
          </div>
        </div>
      );
    ...
    case 'location':
      return (
        <div className="text-center space-y-6 animate-in zoom-in duration-500">
          ...
          <h2 className="font-serif text-3xl text-[#3E2723]">{resolveVenue(data).name}</h2>
          <p className="text-gray-600 max-w-md mx-auto leading-relaxed">{resolveVenue(data).address}</p>
          <a href={resolveVenue(data).mapsLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 bg-[#8B4513] text-white px-8 py-3 rounded-full hover:bg-[#6d360f] transition shadow-lg">
          ...
          <div className="space-y-4">
            {resolveBanks(data).map((bank, i) => (
              <div key={i} className="bg-white p-6 border border-gray-200 shadow-lg relative">
                ...
                <button onClick={() => copyRek(bank.number)} className="text-[#8FBC8F] text-xs font-bold hover:underline">COPY NUMBER</button>
              </div>
            ))}
          </div>
        </div>
...
const ArtFrame = ({  id, type, title, subtitle, size = 'square', onClick, groom, bride  }: { id: string; type: string; title: string; subtitle: string; size?: 'square' | 'tall' | 'wide' | 'custom'; onClick?: (id: string) => void; groom?: string; bride?: string })  => {
...
export default function ArtGalleryTheme({
  ...
  // Audio Control — logika dipusatkan di useAudioToggle
  const { playing: isAudioPlaying, setPlaying: setIsAudioPlaying, toggle: toggleAudioBase } = useAudioToggle(audioRef);
  useEffect(() => {
  ...
  // Tombol musik berada di dalam kartu yang bisa diklik — hentikan propagasi
  // agar klik tombol tidak ikut membuka kartu.
