...
const PostHeader = ({  locationText, username, photos  }: { locationText?: string; username: string; photos: { profile: string } })  => (
...
const ActionBar = ({  id, liked, handleLike  }: { id: string; liked: boolean; handleLike: (id: string) => void })  => (
...
export default function InstaTheme({ groom, bride, date, guestName, data }: TemplateProps) {
    // --- STATE ---
    ...
    const location = resolveVenue(data).name;

    const photos = {
        profile: data?.cover_photo || DEFAULT_PHOTOS.cover,
        feed1: data?.cover_photo || DEFAULT_PHOTOS.gallery[0],
        ...resolvePhotos(data),
        gift_bg: "https://r2.loverse.id/defaults/img/9a293ef1a6.jpg",
    ...
    const formattedAkadDate = schedule.akadDate ? formatDate(schedule.akadDate, 'en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : formattedDate;
    const formattedResepsiDate = schedule.resepsiDate ? formatDate(schedule.resepsiDate, 'en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : formattedDate;
...
const { playing: isPlaying, toggle: toggleAudio } = useAudioToggle(audioRef);
                            ...
                            <div className="space-y-4 w-full">
                                <div className="flex items-center gap-3 text-left">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500">{formattedAkadDate}</p>
                                        <p className="text-xs text-gray-500">{schedule.akadTime}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3 text-left">
                                    <div className="bg-gray-100 p-2 rounded-full"><Calendar size={16} /></div>
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500">{formattedResepsiDate}</p>
                                        <p className="text-xs text-gray-500">{schedule.resepsiTime}</p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                        <div className="bg-white w-full max-w-sm h-[400px] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
                            <div className="flex items-center justify-between p-3 border-b border-gray-100 bg-white sticky top-0">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            <p className="font-bold text-sm leading-none text-black">{username}</p>
                            ...
                            <div className="p-3 border-t border-gray-100 flex items-center gap-3 bg-white">
                                ...
                                <div className="bg-gray-100 h-9 rounded-full flex-1 px-4 flex items-center text-gray-400 text-sm cursor-text">
                                    Message...
                                </div>
