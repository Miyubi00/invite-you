...
export default function RusticTheme({ groom, bride, date, guestName, data }: TemplateProps) {
  ...
  const { isOpen, open: openInvitation, playing: audioPlaying, toggle: toggleAudio } = useOpenInvitation(audioRef, 0);
      ...
      <div className={`transition-opacity duration-1000 delay-300 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="max-w-4xl mx-auto bg-white rounded-[3rem] shadow-lg overflow-hidden border border-[#F0EBE5] relative p-10 md:p-16 text-center">
               ...
               <div className="flex flex-col md:flex-row gap-8 justify-center items-center mb-12">
                  ...
                  <div className="bg-[#F9F7F5] p-8 rounded-[2rem] border border-[#E3DDD5] w-full max-w-sm hover:-translate-y-1 transition duration-300">
                     ...
                     <p className="text-sm text-[#8C8075] mb-1">{formattedAkadDate}</p>
                     <div className="flex items-center justify-center gap-2 text-[#8C8075] mb-4 text-sm font-bold">
                        <Clock className="w-4 h-4" /> <span>{schedule.akadTime}</span>
                     </div>
                  </div>
                  ...
                  <div className="bg-[#712E1E] p-8 rounded-[2rem] border border-[#712E1E] w-full max-w-sm text-white shadow-lg hover:-translate-y-1 transition duration-300 relative overflow-hidden">
                     ...
                     <p className="text-sm text-white/80 mb-1">{formattedResepsiDate}</p>
                     <div className="flex items-center justify-center gap-2 text-white/90 mb-4 text-sm font-bold">
                        <Clock className="w-4 h-4" /> <span>{schedule.resepsiTime}</span>
                     </div>
                  </div>

                  {/* Venue */}
                  <div className="bg-white p-8 rounded-[2rem] border border-[#E3DDD5] w-full max-w-sm text-center shadow-sm hover:-translate-y-1 transition duration-300">
                     <div className="w-12 h-12 bg-[#D4C3B7]/30 text-[#712E1E] rounded-full flex items-center justify-center mx-auto mb-4">
                        <MapPin className="w-6 h-6" />
                     </div>
                     <h3 className="font-bold text-xl mb-2 text-[#5D534A]">Lokasi</h3>
                     <p className="text-sm text-[#8C8075] mb-1">{venue.name}</p>
                     <p className="text-xs text-[#8C8075] opacity-70 mb-4">{venue.address}</p>
                     <a href={venue.mapsLink} target="_blank" className="inline-flex items-center gap-2 bg-[#5D534A] text-white px-6 py-2 rounded-full hover:bg-[#3E3730] transition shadow-md text-sm font-medium">
        ...
        <section className="py-20 px-6 bg-white rounded-t-[3rem] shadow-[0_-10px_40px_rgba(0,0,0,0.05)] text-center relative border-t border-[#F0EBE5]">
              ...
              <div key={idx} className="bg-[#F9F7F5] p-6 rounded-2xl w-full max-w-xs shadow-sm border border-[#E3DDD5] relative group hover:-translate-y-1 transition duration-300">
                ...
                <div className="text-left relative z-10">
                   ...
                   <div className="flex items-center justify-between bg-white p-3 rounded-lg mb-2 border border-[#E3DDD5]">
                      ...
                      <button onClick={() => copyRek(bank.number)} className="p-1 hover:bg-gray-100 rounded transition"><Copy className="w-4 h-4 text-[#712E1E]"/></button>
                   </div>
          ...
          <footer className="pt-10 border-t border-[#F0EBE5]">
             ...
             <p className="text-xs text-[#8C8075] font-bold tracking-widest">Created with <Heart className="w-3 h-3 inline text-red-300 fill-current"/> by LoVerse</p>
          </footer>
        </section>
...
function ToastIcon(props: React.SVGProps<SVGSVGElement>) {
...
function DecorFlower(props: React.SVGProps<SVGSVGElement>) {
...
// Dekorasi Ranting
function DecorBranch(props: React.SVGProps<SVGSVGElement>) {
