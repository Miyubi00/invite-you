...
export default function ModernDarkTheme({ groom, bride, date, guestName, data }: TemplateProps) {
  ...
  const formattedAkadDate = schedule.akadDate ? formatDate(schedule.akadDate, 'en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).toUpperCase() : formattedDate;
  const formattedResepsiDate = schedule.resepsiDate ? formatDate(schedule.resepsiDate, 'en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).toUpperCase() : formattedDate;
  const details = {
           ...
           <div className="max-w-4xl mx-auto px-6 relative">
              ...
              <div className="flex flex-col md:flex-row items-center mb-16 relative">
                 <div className="md:w-1/2 p-6 md:text-right md:pr-12 order-2 md:order-1">
                    ...
                    <p className="text-sm text-[#D4AF37] font-modern mb-1">{formattedAkadDate}</p>
                    <p className="text-xl text-white font-modern mb-1">{schedule.akadTime}</p>
                 </div>
              ...
              <div className="flex flex-col md:flex-row items-center relative">
                 ...
                 <div className="md:w-1/2 p-6 md:pl-12 order-3 md:order-2">
                    ...
                    <p className="text-sm text-[#D4AF37] font-modern mb-1">{formattedResepsiDate}</p>
                    <p className="text-xl text-white font-modern mb-1">{schedule.resepsiTime}</p>
                 </div>
              </div>

              {/* Venue */}
              <div className="flex flex-col md:flex-row items-center relative">
                 <div className="md:w-1/2 p-6 md:text-right md:pr-12 order-2 md:order-1 hidden md:block"></div>
                 <div className="absolute left-6 md:left-1/2 -translate-x-1/2 w-4 h-4 bg-[#3B82F6] rounded-full border-4 border-black z-10"></div>
                 <div className="md:w-1/2 p-6 md:pl-12 order-3 md:order-2">
                    <h3 className="font-luxury text-3xl text-blue-400 mb-2">Venue</h3>
                    <p className="text-sm text-gray-500 mb-1">{venue.name}</p>
                    <p className="text-xs text-gray-600 mb-4">{venue.address}</p>
                    <div>
                        <a href={venue.mapsLink} target="_blank" className="inline-flex items-center gap-2 text-xs border-b border-[#3B82F6] pb-1 hover:text-blue-400 transition">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                 {banks.map((bank, idx) => (
                    <div key={idx} className="bg-[#1A1A1A] p-8 border border-white/5 hover:border-[#D4AF37] transition duration-500 text-left relative group">
                        ...
                        <button onClick={() => copyRek(bank.number)} className="text-[#D4AF37] text-xs font-bold border border-[#D4AF37] px-4 py-2 hover:bg-[#D4AF37] hover:text-black transition uppercase">
                           Copy Number
                        </button>
                    </div>
                 ))}
