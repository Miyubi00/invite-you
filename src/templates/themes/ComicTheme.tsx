...
export default function ComicTheme({ groom, bride, date, guestName, data, onRsvpSubmit, submittedData }: TemplateProps) {
                ...
                <div className="relative">
                    ...
                    <div className="comic-panel-dark p-6">
                        ...
                        <div className="space-y-6">
                            ...
                            {/* Venue */}
                            <div className="bg-white text-black border-2 border-black p-4 relative shadow-[4px_4px_0_#3B82F6]">
                                <div className="absolute -top-3 -left-3 bg-black text-white px-2 py-1 font-comic-head text-sm rotate-[-5deg]">VENUE</div>
                                <h3 className="font-comic-head text-2xl">LOCATION</h3>
                                <div className="mt-2 border-t-2 border-dashed border-gray-300 pt-2 text-sm font-hand">
                                    <div className="font-bold">
                                        {resolveVenue(data).name}
                                    </div>
                                    <div className="opacity-80">
                                        {resolveVenue(data).address}
                                    </div>
                                </div>
                                <a href={resolveVenue(data).mapsLink} target="_blank" className="mt-4 block bg-[#1D3557] text-white text-center py-2 font-comic-head border-2 border-black shadow-[2px_2px_0_white] hover:translate-y-[2px] hover:shadow-none transition-all">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <select value={rsvpStatus} onChange={(e) => setRsvpStatus(e.target.value as RsvpStatus)} className="comic-input cursor-pointer">
                            ...
                            {rsvpStatus === 'hadir' && (
                                <div>
                                    ...
                                    <select value={rsvpPax} onChange={(e) => setRsvpPax(Number(e.target.value))} className="comic-input cursor-pointer">
                                    ...
                                    <div className="flex-1">
                                        <div className="chat-bubble-comic">
                                            <div className="flex justify-between items-start mb-1">
                                                ...
                                                <span className={`text-[10px] px-2 border border-black font-bold uppercase ${item.status === 'hadir' ? 'bg-[#A8DADC]' : item.status === 'tidak_hadir' ? 'bg-[#FF8A80]' : 'bg-[#F4D35E]'}`}>
...
function ScoreBox({ val, label }: { val: number; label: string }) {
    return (
