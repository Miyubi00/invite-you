...
export default function LuxuryGoldTheme({ groom, bride, date, guestName, data }: TemplateProps) {
  const audioRef = useRef<HTMLAudioElement | null>(null);
  const audioUrl = data?.audio_url || '';
  const copyRek = useCopyToClipboard();
  const schedule = resolveSchedule(data, date);
  const formattedDate = date ? new Date(date).toLocaleDateString('en-US', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) : '';
  const formattedAkadDate = schedule.akadDate ? formatDate(schedule.akadDate, 'en-US', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) : formattedDate;
  const formattedResepsiDate = schedule.resepsiDate ? formatDate(schedule.resepsiDate, 'en-US', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) : formattedDate;
    ...
    <div className="bg-[#0a0f1c] text-[#e2e8f0] min-h-screen font-sans overflow-x-hidden selection:bg-amber-900 selection:text-white">
      ...
      <div className={`fixed inset-0 z-50 bg-[#05080f] flex flex-col items-center justify-center transition-opacity duration-1000 ${isOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
        ...
        <div className="relative z-10 text-center p-8 border border-amber-500/30 bg-[#0a0f1c]/80 backdrop-blur-sm max-w-sm w-full mx-4 rounded-t-[100px] rounded-b-[100px] shadow-[0_0_50px_rgba(191,149,63,0.1)]">
            ...
            <button onClick={open} className="px-8 py-3 bg-gradient-to-r from-amber-700 to-amber-600 text-white font-modern text-sm tracking-widest uppercase rounded hover:scale-105 transition shadow-[0_0_20px_rgba(180,83,9,0.4)]">
                Open Invitation
            </button>
        </div>
      </div>
            ...
            <div className="container mx-auto px-4 relative z-10 flex flex-col md:flex-row justify-center gap-10">
                ...
                <div className="bg-[#0f1623] border border-amber-500/20 p-8 rounded-none relative group hover:border-amber-500/50 transition duration-500 w-full md:w-1/3 text-center">
                    ...
                    <div className="space-y-3 font-modern text-sm text-slate-400">
                        <p className="text-amber-200">{formattedAkadDate}</p>
                        <p className="text-amber-200">{schedule.akadTime}</p>
                    </div>
                </div>
                ...
                <div className="bg-gradient-to-b from-amber-900/20 to-[#0f1623] border border-amber-500/40 p-10 rounded-none relative group w-full md:w-1/3 text-center transform md:-translate-y-6 shadow-[0_0_30px_rgba(180,83,9,0.1)]">
                    ...
                    <div className="space-y-3 font-modern text-sm text-slate-300">
                        <p className="text-amber-200 text-lg">{formattedResepsiDate}</p>
                        <p className="text-amber-200 text-lg">{schedule.resepsiTime}</p>
                    </div>
                </div>

                {/* Card 3: Venue */}
                <div className="bg-gradient-to-b from-blue-900/20 to-[#0f1623] border border-blue-500/40 p-10 rounded-none relative group w-full md:w-1/3 text-center transform md:-translate-y-6 shadow-[0_0_30px_rgba(59,130,246,0.1)]">
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#0a0f1c] px-4 text-blue-400 font-royal tracking-widest border border-blue-500/30">VENUE</div>
                    <h3 className="text-3xl font-royal text-white mb-4 mt-2">Lokasi</h3>
                    <div className="space-y-3 font-modern text-sm text-slate-300">
                        <p>{resolveVenue(data).name}</p>
                        <p className="text-xs opacity-60">{resolveVenue(data).address}</p>
                        <a href={resolveVenue(data).mapsLink} className="inline-flex items-center gap-2 text-blue-600 text-xs mt-2 hover:text-blue-400"><MapPin size={12}/> View Map</a>
                    </div>
                </div>
            </div>
        </section>
                         ...
                         <div className="p-6 h-full flex flex-col justify-between relative z-10">
                             ...
                             <div>
                                 <div className="flex items-center gap-3 mb-1">
                                    ...
                                    <button onClick={() => copyRek(bank.number)} className="text-amber-600 hover:text-amber-400"><Copy size={14}/></button>
                                 </div>
