...
import type { RsvpPayload, TemplateData, TemplateProps } from '../../types/template';
import type { BankAccount, RsvpRow } from '../../types/database';
import { DEFAULT_PHOTOS, formatDate, resolveBanks, resolvePhotos, resolveVenue, resolveSchedule } from '../../utils/templateHelpers';
...
export default function ChatBubbleTheme({ groom, bride, date, data, onRsvpSubmit, submittedData }: TemplateProps) {
    ...
    const [rsvpData, setRsvpData] = useState<RsvpPayload>({ status: 'hadir', pax: 1, message: '' });
    const [isSending, setIsSending] = useState(false);

    // --- DATA ---
    const photos = {
    ...resolvePhotos(data),
    bg: data?.cover_photo || DEFAULT_PHOTOS.cover
};
...
function AnswerContent({  id, data, groom, bride, date, submittedData, rsvpStep, setRsvpStep, rsvpData, setRsvpData, handleSendRsvp, isSending  }: { id: string; data?: TemplateData; groom: string; bride: string; date: string; submittedData?: RsvpRow | null; rsvpStep: number; setRsvpStep: (step: number) => void; rsvpData: RsvpPayload; setRsvpData: Dispatch<SetStateAction<RsvpPayload>>; handleSendRsvp: () => Promise<void>; isSending: boolean }) {
    const copyRek = useCopyToClipboard();
    const schedule = resolveSchedule(data, date);
    const fallbackDate = formatDate(date, 'id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
    ...
    if (id === 'date') {
        return <div className="space-y-2"><p>Insya Allah acara kami akan dilaksanakan pada:</p><p>Akad nikah pada <strong>{formattedAkadDate}</strong> pukul <strong>{schedule.akadTime}</strong>.</p><p>Resepsi pada <strong>{formattedResepsiDate}</strong> pukul <strong>{schedule.resepsiTime}</strong>. Jangan telat ya! 😉</p></div>;
    }
    if (id === 'location') {
        return <div className="space-y-3"><p>Acaranya bertempat di:</p><div className="bg-gray-50 p-3 rounded-lg border border-gray-200"><strong className="block text-gray-800">{resolveVenue(data).name}</strong><span className="text-xs text-gray-500">{resolveVenue(data).address}</span></div><a href={resolveVenue(data).mapsLink} target="_blank" className="inline-flex items-center gap-1 text-blue-500 font-bold hover:underline"><MapPin size={14} /> Buka Google Maps</a></div>;
    }
    if (id === 'couple') {
    ...
    if (id === 'gift') {
        return <div><p className="mb-3">Wah, makasih banget lho! Kehadiranmu aja udah kado terindah buat kami. 🥰</p><p className="text-xs mb-2">Tapi kalau mau kirim tanda kasih, bisa kesini ya:</p><div className="space-y-2">{resolveBanks(data).map((bank, i) => (<div key={i} className="bg-gray-50 p-2 rounded border border-gray-200 flex justify-between items-center"><div><span className="font-bold text-xs block">{bank.bank}</span><span className="text-xs text-gray-500">{bank.number}</span></div><button onClick={() => copyRek(bank.number)} className="text-blue-500 text-xs font-bold">Salin</button></div>))}</div></div>;
    }
    if (id === 'gallery') {
        return <div className="space-y-2"><p>Ini beberapa momen keseruan kami prewedding kemarin! 📸</p><div className="grid grid-cols-2 gap-2 mt-2">{(data?.gallery || []).slice(0, 4).map((img: string, i: number) => (<img key={i} src={img} className="w-full h-24 object-cover rounded-lg" />))}</div></div>;
    }
                    ...
                    <div>
                        {rsvpStep === 0 && (
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    <button onClick={() => { setRsvpData({ ...rsvpData, status: 'hadir' }); setRsvpStep(1); }} className="bg-blue-100 text-blue-600 py-2 rounded-lg font-bold text-xs hover:bg-blue-200 transition">Hadir 🙋</button>
                                    <button onClick={() => { setRsvpData({ ...rsvpData, status: 'ragu' }); setRsvpStep(1); }} className="bg-yellow-100 text-yellow-600 py-2 rounded-lg font-bold text-xs hover:bg-yellow-200 transition">Ragu 🤔</button>
                                    <button onClick={() => { setRsvpData({ ...rsvpData, status: 'tidak_hadir' }); setRsvpStep(1); }} className="bg-red-100 text-red-600 py-2 rounded-lg font-bold text-xs hover:bg-red-200 transition">Tidak 🙅</button>
                                </div>
                            </div>
                        )}
                        ...
                        {rsvpStep === 1 && (
                            <div className="space-y-3 animate-fade-in-up">
                                <p>Baik, {rsvpData.status === 'hadir' ? 'kamu mau datang berapa orang?' : rsvpData.status === 'ragu' ? 'belum pasti ya, nggak apa-apa! Tulis pesan untuk kami dulu' : 'yah sayang sekali. Ada pesan untuk kami?'} 👇</p>
                                <div className="space-y-2">
                                    {rsvpData.status === 'hadir' && (
                                        <select
                                            value={rsvpData.pax}
                                            onChange={(e) => setRsvpData({ ...rsvpData, pax: Number(e.target.value) })}
                                    ...
                                    {/* Wrapper agar textarea dan counter menempel */}
                                    <div>
                                        <textarea
                                        ...
                                        <div className="text-right text-[10px] text-gray-400 mt-1 mr-1">
                                            {String(rsvpData.message || '').length}/100
                                        </div>
                                    </div>
                                </div>
                                <button
                ...
                <div className="border-t border-gray-200 pt-3">
                    ...
                    <div className="space-y-3 max-h-[180px] overflow-y-auto chat-scroll pr-1">
                        {(data?.rsvps || []).length === 0 ? (
                            ...
                            (data?.rsvps || []).map((item: RsvpRow, idx: number) => (
