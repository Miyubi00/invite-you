                ...
                >
                    ...
                    <div className="absolute top-[-350px] left-[-500px] w-[300px] -translate-x-1/2 -translate-y-1/2 float-3">
                        <div className="cloud-card">
                            ...
                            <div className="text-left bg-white/50 rounded-xl p-3 text-xs space-y-3">
                                <div>
                                    <div className="font-bold text-sky-600">Akad Nikah</div>
                                    <div className="text-slate-500">{formattedAkadDate}</div>
                                    <div className="text-slate-600 font-bold">{schedule.akadTime}</div>
                                </div>
                                <div className="border-t border-slate-200"></div>
                                <div>
                                    <div className="font-bold text-pink-500">Resepsi</div>
                                    <div className="text-slate-500">{formattedResepsiDate}</div>
                                    <div className="text-slate-600 font-bold">{schedule.resepsiTime}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="absolute top-[-350px] left-[500px] w-[300px] -translate-x-1/2 -translate-y-1/2 float-3">
                        <div className="cloud-card">
                            ...
                            <p className="font-body text-xs text-slate-500 mt-1 mb-3 line-clamp-2">{resolveVenue(data).name}</p>
                            <a href={resolveVenue(data).mapsLink} target="_blank" onPointerDown={(e) => e.stopPropagation()} className="bg-green-500 text-white px-4 py-2 rounded-full text-xs font-bold shadow-md hover:bg-green-600 transition flex items-center justify-center gap-1 w-full">
                            ...
                            <div className="space-y-2">
                                {resolveBanks(data).map((bank, i) => (
                                    <div key={i} className="bg-white p-2 rounded-lg border border-purple-100 text-left flex justify-between items-center shadow-sm">
                                        ...
                                        <button onPointerDown={(e) => { e.stopPropagation(); void copyRek(bank.number); }} className="bg-purple-100 p-1.5 rounded-md text-purple-600 hover:bg-purple-200">
                                            <Copy size={14} />
                                        </button>
                                    </div>
                                ))}
                            ...
                            {submittedData ? (
                                ...
                                <form onSubmit={handleRsvpSubmit} className="space-y-3 text-left">
                                    <div className="space-y-1">
                                        ...
                                        <select value={rsvpStatus} onChange={(e) => setRsvpStatus(e.target.value as RsvpStatus)} className="w-full text-xs p-3 rounded-xl border border-sky-100 focus:outline-none focus:border-sky-300 bg-sky-50/30 font-bold text-slate-600">
                                    ...
                                    {rsvpStatus === 'hadir' && (
                                        <div className="space-y-1">
                                            ...
                                            <select value={rsvpPax} onChange={(e) => setRsvpPax(Number(e.target.value))} className="w-full text-xs p-3 rounded-xl border border-sky-100 focus:outline-none focus:border-sky-300 bg-sky-50/30">
