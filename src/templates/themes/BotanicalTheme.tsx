...
export default function CelestialTheme({ groom, bride, date, guestName, data }: TemplateProps) {
            ...
            <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-6 relative z-10">
                ...
                <div className="glass p-8 rounded-2xl text-center border-t-4 border-t-[#fbbf24]">
                    ...
                    <div className="space-y-4 font-modern text-sm text-gray-300">
                         <div className="flex items-center justify-center gap-2"><Calendar size={16} className="text-[#fbbf24]"/> {formattedAkadDate}</div>
                         <div className="flex items-center justify-center gap-2"><Clock size={16} className="text-[#fbbf24]"/> {resolveSchedule(data).akadTime}</div>
                    </div>
                </div>
                ...
                <div className="glass p-8 rounded-2xl text-center border-t-4 border-t-purple-500">
                    ...
                    <div className="space-y-4 font-modern text-sm text-gray-300">
                         <div className="flex items-center justify-center gap-2"><Calendar size={16} className="text-purple-400"/> {formattedResepsiDate}</div>
                         <div className="flex items-center justify-center gap-2"><Clock size={16} className="text-purple-400"/> {resolveSchedule(data).resepsiTime}</div>
                    </div>
                </div>
            </div>

            {/* Venue */}
            <div className="glass p-8 rounded-2xl text-center border-t-4 border-t-blue-500 mt-6">
                 <div className="w-12 h-12 mx-auto bg-blue-500/10 rounded-full flex items-center justify-center text-blue-400 mb-4">
                    <MapPin size={24} />
                </div>
                <h3 className="font-modern font-bold text-2xl text-white mb-6">Lokasi</h3>
                <div className="space-y-4 font-modern text-sm text-gray-300">
                     <div className="flex items-center justify-center gap-2 px-6"><MapPin size={16} className="text-blue-400 shrink-0"/> {resolveVenue(data).name} - {resolveVenue(data).address}</div>
                     <a href={resolveVenue(data).mapsLink} target="_blank" className="inline-block mt-4 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-6 py-2 rounded-full transition">
                    ...
                    {banks.map((bank, i) => (
                        <div key={i} className="bg-[#0f172a]/80 p-4 rounded-xl border border-white/10 flex flex-col md:flex-row items-center justify-between gap-4">
                             ...
                             <div className="flex items-center gap-3 bg-white/5 px-4 py-2 rounded-lg border border-white/5">
                                 ...
                                 <button onClick={() => copyRek(bank.number)} className="text-[#fbbf24] hover:text-white transition">
                                     <Copy size={16}/>
                                 </button>
                             </div>
                        </div>
