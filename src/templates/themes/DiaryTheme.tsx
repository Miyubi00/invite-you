...
export default function HandwrittenDiaryTheme({ groom, bride, date, guestName, data, onRsvpSubmit, submittedData }: TemplateProps) {
          ...
          <div className="border-2 border-[#2B2B2B] p-8 relative bg-white/50 transform -rotate-1 shadow-sm">
            ...
            <div className="space-y-8">
              <div className="flex gap-4 items-start">
                ...
                <div><h3 className="font-title text-2xl font-bold">Akad Nikah</h3><p className="font-body text-lg">{formattedAkadDate} • {schedule.akadTime}</p></div>
              </div>
              <div className="flex gap-4 items-start">
                <div className="mt-1 bg-[#FCE7F3] text-[#831843] p-1 rounded-full border border-[#831843]"><Check size={20} /></div>
                <div><h3 className="font-title text-2xl font-bold">Reception Party</h3><p className="font-body text-lg">{formattedResepsiDate} • {schedule.resepsiTime}</p></div>
              </div>
              <div className="flex gap-4 items-start">
                <div className="mt-1 bg-[#DBEAFE] text-[#1E40AF] p-1 rounded-full border border-[#1E40AF]"><MapPin size={20} /></div>
                <div><h3 className="font-title text-2xl font-bold">Lokasi</h3><div className="mt-1 font-doodle text-sm text-gray-500"><p className="font-bold text-gray-700">{resolveVenue(data).name}</p><p className="opacity-80">{resolveVenue(data).address}</p></div><a href={resolveVenue(data).mapsLink} target="_blank" className="inline-block mt-3 border-b border-[#2B2B2B] text-[#2B2B2B] font-body hover:text-[#8B5E3C] transition"><MapPin className="inline w-4 h-4 mr-1" /> See Location on Map</a></div>
              </div>
            </div>
          </div>
        </section>
            ...
            <div className="space-y-4">
              {banks.map((bank, i) => (
                <div key={i} className="border-b border-[#D97706]/30 pb-2">
                  ...
                  <div className="flex justify-between items-center mt-1"><span className="font-body text-lg text-[#78350F]">{bank.number}</span><button onClick={() => copyRek(bank.number)} className="text-[#92400E] hover:scale-110 transition"><Copy size={18} /></button></div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={rsvpStatus} onChange={(e) => setRsvpStatus(e.target.value as RsvpStatus)} className="w-full bg-white border-2 border-[#2B2B2B] rounded-lg p-2 font-body text-lg focus:outline-none">
                  ...
                  {rsvpStatus === 'hadir' && (
                    <div>
                      ...
                      <select value={rsvpPax} onChange={(e) => setRsvpPax(Number(e.target.value))} className="w-full bg-white border-2 border-[#2B2B2B] rounded-lg p-2 font-body text-lg focus:outline-none">
                  ...
                  (data?.rsvps || []).map((item, idx) => (
                    <div key={idx} className="relative pl-6 border-l-2 border-[#D4C4B7]">
                      ...
                      <div className="flex justify-between items-baseline mb-1">
                        ...
                        <span className={`text-xs font-body uppercase tracking-wider ${item.status === 'hadir' ? 'text-green-600' : item.status === 'ragu' ? 'text-yellow-600' : 'text-red-500'}`}>{item.status === 'hadir' ? 'Will Attend' : item.status === 'ragu' ? 'Maybe' : "Can't Come"}</span>
                      </div>
...
function DoodleStar(props: React.SVGProps<SVGSVGElement>) {
  return (
...
function DoodleHeart(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 100 100" fill="currentColor" {...props}>
