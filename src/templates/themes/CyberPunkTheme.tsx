...
export default function CyberpunkTheme({ groom, bride, date, guestName, data }: TemplateProps) {
  ...
  const formattedAkadDate = schedule.akadDate ? formatDate(schedule.akadDate, 'en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' }).replace(/\//g, '.') : formattedDate;
  const formattedResepsiDate = schedule.resepsiDate ? formatDate(schedule.resepsiDate, 'en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' }).replace(/\//g, '.') : formattedDate;

  // --- LOGIC ---
  const timeLeft = useCountdown(date);

  // Pola bersama (dedup 3.2): audio boot sinkron hasil promise play().
  const { open: openBootAudio, playing: isPlaying, toggle: toggleAudio } =
    ...
    <div className="bg-[#05070F] text-[#E0E0E0] min-h-screen relative overflow-x-hidden font-cyber selection:bg-[#00F5FF] selection:text-black">
      ...
      <div className={`transition-opacity duration-1000 ${booted ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="relative z-10 max-w-lg mx-auto pb-24">
              ...
              {/* 1. HERO (DATA STREAM) */}
              <header className="pt-12 px-6 mb-12 text-center relative overflow-hidden">
                  {/* COVER PHOTO BACKGROUND (foto wedding klien) */}
                  <div className="absolute inset-0 z-0">
                      <img src={photos.cover} className="w-full h-full object-cover opacity-25 saturate-150" alt="Cover" />
                      <div className="absolute inset-0 bg-gradient-to-b from-[#05070F]/50 via-[#05070F]/60 to-[#05070F]"></div>
                  </div>
                      ...
                      <div className="space-y-6 mt-2">
                          <div className="flex gap-4 items-start">
                              ...
                              <div>
                                  ...
                                  <div className="flex gap-4 mt-1 text-xs text-[#22FF88]">
                                      <span>AKAD: {formattedAkadDate} {schedule.akadTime}</span>
                                      <span>|</span>
                                      <span>PARTY: {formattedResepsiDate} {schedule.resepsiTime}</span>
                                  </div>
                              </div>
                          </div>
                          ...
                          <div className="flex gap-4 items-start">
                              ...
                              <div>
                                  ...
                                  <p className="font-cyber-body text-gray-300 text-sm mb-3">{resolveVenue(data).name} <br/> {resolveVenue(data).address}</p>
                                  <a href={resolveVenue(data).mapsLink} target="_blank" className="inline-block bg-[#22FF88] text-black px-4 py-1 text-sm font-bold hover:bg-white transition skew-x-[-10deg]">
                                      INITIATE_GPS
                                  </a>
                              </div>
                          </div>
                      ...
                      <div className="space-y-4 relative z-10">
                          {banks.map((bank, i) => (
                              <div key={i} className="bg-black/50 border border-gray-700 p-4 flex justify-between items-center group hover:border-[#FF2BD6] transition">
                                  ...
                                  <button onClick={() => copyRek(bank.number)} className="text-[#FF2BD6] hover:text-white transition">
                                      <Copy size={20}/>
                                  </button>
                              </div>
                          ))}
...
// SUB COMPONENT
function TimeDigit({ val, label, color }: { val: number; label: string; color: string }) {
    return (
