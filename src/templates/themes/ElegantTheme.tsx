...
import { DEFAULT_PHOTOS, formatDate, resolveBanks, resolveGallery, resolvePhotos, resolveVenue, resolveSchedule } from '../../utils/templateHelpers';

export default function ElegantResponsiveTheme({ groom, bride, date, guestName, data }: TemplateProps) {
  ...
  const photos = {
    ...resolvePhotos(data),
    header: data?.cover_photo || DEFAULT_PHOTOS.cover,
  };
  ...
  // menggantikan toggleAudio lokal dengan state audio manual.
  const { isOpen, open: openInvitation, playing: audioPlaying, toggle: toggleAudio } =
    useOpenInvitation(audioRef, 0);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto">
                    ...
                    <div className="bg-white/5 border border-white/10 p-6 md:p-10 rounded-xl hover:bg-white/10 transition duration-300 relative overflow-hidden">
                        ...
                        <div className="space-y-4 text-slate-300 text-sm md:text-base">
                            <div className="flex items-center gap-3"><Calendar className="w-5 h-5 text-emerald-500 shrink-0"/> <span className="font-bold text-white">{formattedAkadDate}</span></div>
                            <div className="flex items-center gap-3"><Clock className="w-5 h-5 text-emerald-500 shrink-0"/> <span>{schedule.akadTime}</span></div>
                            <div className="flex items-start gap-3"><MapPin className="w-5 h-5 text-emerald-500 mt-1 shrink-0"/> <span>{venue.name}<br/> <span className="text-xs opacity-70">{venue.address}</span></span></div>
                        </div>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-emerald-900 to-slate-900 border border-emerald-800 p-6 md:p-10 rounded-xl relative overflow-hidden shadow-2xl">
                        ...
                        <div className="space-y-4 text-emerald-100 relative z-10 text-sm md:text-base">
                            <div className="flex items-center gap-3"><Calendar className="w-5 h-5 text-emerald-400 shrink-0"/> <span className="font-bold text-white">{formattedResepsiDate}</span></div>
                            <div className="flex items-center gap-3"><Clock className="w-5 h-5 text-emerald-400 shrink-0"/> <span>{schedule.resepsiTime}</span></div>
                        </div>
                    </div>                </div>

                {/* Venue */}
                <div className="max-w-5xl mx-auto mt-8">
                    <div className="bg-white/5 border border-white/10 p-6 md:p-10 rounded-xl text-center">
                        <MapPin className="w-8 h-8 text-emerald-400 mx-auto mb-4"/>
                        <h3 className="font-serif text-2xl text-emerald-400 mb-2">Lokasi</h3>
                        <p className="text-white font-bold mb-1">{venue.name}</p>
                        <p className="text-sm text-gray-400 mb-4">{venue.address}</p>
                        <a href={venue.mapsLink} target="_blank" className="inline-block py-3 px-8 bg-emerald-600 hover:bg-emerald-500 text-white rounded font-bold transition text-sm tracking-wider">
                            VIEW LOCATION
                        </a>
                    </div>
                </div>
                        ...
                        <div key={idx} className="w-full max-w-sm bg-gradient-to-r from-slate-800 to-slate-700 text-white rounded-xl p-6 shadow-lg relative overflow-hidden group transform transition md:hover:-translate-y-2">
                             ...
                             <div className="text-left relative z-10">
                                 ...
                                 <div className="flex items-center justify-between bg-white/5 p-2 rounded border border-white/10">
                                     ...
                                     <button onClick={() => copyRek(bank.number)} className="p-2 bg-white/10 rounded hover:bg-white/20 transition shrink-0 ml-2">
