...
import { DEFAULT_PHOTOS, formatDate, resolveBanks, resolveVenue, resolveSchedule } from '../../utils/templateHelpers';
...
const NavigationBar = ({  lightMode = false, onBack, onHome  }: { lightMode?: boolean; onBack: () => void; onHome: () => void })  => (
...
const AppIcon = ({  icon: IconComponent, label, color, onClick  }: { icon: LucideIcon; label: string; color: string; onClick: () => void })  => {
...
const CoupleApp = ({  data, groom, bride  }: { data?: TemplateData; groom: string; bride: string })  => (
        ...
        <div className="px-6 space-y-8">
            ...
            <div className="flex flex-col items-center text-center">
                <div className="w-32 h-32 rounded-full border-4 border-white shadow-xl overflow-hidden mb-4 bg-gray-200">
                    <img src={data?.groom_photo || DEFAULT_PHOTOS.groom} className="w-full h-full object-cover" alt="Groom" />
                </div>
            ...
            <div className="flex flex-col items-center text-center">
                <div className="w-32 h-32 rounded-full border-4 border-white shadow-xl overflow-hidden mb-4 bg-gray-200">
                    <img src={data?.bride_photo || DEFAULT_PHOTOS.bride} className="w-full h-full object-cover" alt="Bride" />
                </div>
...
const EventApp = ({  data, weddingDateString, date  }: { data?: TemplateData; weddingDateString: string; date?: string })  => {
    const schedule = resolveSchedule(data, date);
    const fallbackDate = weddingDateString;
        ...
        <div className="px-4 space-y-4">
            ...
            <div className="bg-white p-6 rounded-[28px] shadow-sm flex gap-4 items-start">
                ...
                <div>
                    ...
                    <p className="text-blue-600 font-medium mb-1">{formattedAkadDate}</p>
                    <p className="text-blue-600 font-medium mb-1">{schedule.akadTime}</p>
            ...
            <div className="bg-white p-6 rounded-[28px] shadow-sm flex gap-4 items-start">
                ...
                <div>
                    ...
                    <p className="text-purple-600 font-medium mb-1">{formattedResepsiDate}</p>
                    <p className="text-purple-600 font-medium mb-1">{schedule.resepsiTime}</p>
            ...
            <div className="bg-[#E3F2FD] p-6 rounded-[28px] mt-6">
                ...
                <h4 className="font-bold text-xl text-gray-900 mb-2">{resolveVenue(data).name}</h4>
                <p className="text-sm text-gray-600 mb-6">{resolveVenue(data).address}</p>
                <a href={resolveVenue(data).mapsLink} target="_blank" rel="noreferrer" className="block w-full py-3 bg-blue-600 text-white font-bold text-center rounded-xl hover:bg-blue-700 transition">Buka Google Maps</a>
            </div>
        </div>
    </div>
    );
...
const GiftApp = ({  data  }: { data?: TemplateData })  => {
    ...
    <div className="min-h-full bg-[#F3E5F5] pb-24 pt-20 text-gray-800 animate-in fade-in slide-in-from-bottom-8 duration-300">
        <div className="px-4 space-y-4">
            ...
            {resolveBanks(data).length > 0 ? (
                resolveBanks(data).map((bank, index) => (
...
const GalleryApp = ({  data  }: { data?: TemplateData })  => (
...
const RsvpApp = ({  data, submittedData, onRsvpSubmit  }: { data?: TemplateData; submittedData?: RsvpRow | null; onRsvpSubmit?: (payload: RsvpPayload) => Promise<void> })  => {
                    ...
                    {submittedData ? (
                        <div className="text-center py-6">
                            ...
                            <div className="mt-4 p-4 bg-gray-50 rounded-xl text-left border border-gray-100">
                                ...
                                <p className={`font-bold ${submittedData.status === 'hadir' ? 'text-green-600' : submittedData.status === 'ragu' ? 'text-yellow-600' : 'text-red-500'}`}>
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {['hadir', 'ragu', 'tidak_hadir'].map((opt) => (
                                        <button
                                            ...
                                            className={`py-2 rounded-xl text-xs font-bold border-2 transition ${formData.status === opt
...
export default function AndroidHomeTheme({
    ...
    const [activeApp, setActiveApp] = useState<string | null>(null); // null = Home Screen
    const audioRef = useRef<HTMLAudioElement | null>(null);

    const weddingDate = new Date(date);
    const coverPhoto = data?.cover_photo || DEFAULT_PHOTOS.cover;

    // Clock Status Bar - tick berkala agar jam tidak membeku
    useEffect(() => {
    ...
    // Audio Auto Play - logika dipusatkan di useAudioToggle
        ...
        `}</style>
            ...
            <div className="w-full h-[100dvh] md:h-[850px] max-w-[420px] bg-black relative overflow-hidden shadow-2xl rounded-none md:rounded-[45px] border-[0px] md:border-[10px] border-[#202020] ring-1 ring-white/10">
                ...
                <div id="android-screen" className="w-full h-full bg-black overflow-y-auto no-scrollbar relative">
                    ...
                    {activeApp === 'event' && <EventApp data={data} date={date} weddingDateString={weddingDate.toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })} />}
