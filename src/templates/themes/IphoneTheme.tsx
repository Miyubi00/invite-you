                    ...
                    <div className="max-w-4xl mx-auto space-y-6">
                        ...
                        <div className="glass-panel p-6 rounded-[2rem] flex flex-col md:flex-row md:items-center gap-6 relative overflow-hidden">
                            ...
                            <div className="flex-1 pl-4">
                                ...
                                <div className="flex items-center gap-2 text-gray-500 text-sm font-medium mb-4">
                                    <Calendar size={14} /> {formattedAkadDate}
                                </div>
                                <div className="flex items-center gap-2 text-gray-500 text-sm font-medium mb-4">
                                    <Clock size={16} /> {schedule.akadTime}
                                </div>
                            </div>
                        </div>
                        {/* Resepsi */}
                        <div className="glass-panel p-6 rounded-[2rem] flex flex-col md:flex-row md:items-center gap-6 relative overflow-hidden">
                            ...
                            <div className="flex-1 pl-4">
                                ...
                                <div className="flex items-center gap-2 text-gray-500 text-sm font-medium mb-4">
                                    <Calendar size={14} /> {formattedResepsiDate}
                                </div>
                                <div className="flex items-center gap-2 text-gray-500 text-sm font-medium mb-4">
                                    <Clock size={16} /> {schedule.resepsiTime}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="glass-panel p-6 rounded-[2rem] flex flex-col md:flex-row md:items-center gap-6 relative overflow-hidden">
                            <div className="absolute top-0 left-0 w-2 h-full bg-blue-500"></div>
                            <div className="flex-1 pl-4">
                                <span className="inline-block px-3 py-1 rounded-full bg-blue-100 text-blue-600 text-[10px] font-bold uppercase mb-2">Venue</span>
                                <h3 className="text-2xl font-bold text-gray-900 mb-1">Lokasi</h3>
                                <div className="flex items-start gap-2 text-gray-500 text-sm font-medium mb-4">
                                    <MapPin size={16} className="shrink-0 mt-0.5" />
                                    <div className="flex flex-col">
                                        <span className="text-gray-700 font-semibold">{resolveVenue(data).name}</span>
                                        <span className="text-gray-500">{resolveVenue(data).address}</span>
                                    </div>
                                </div>
                            </div>
                            <a href={resolveVenue(data).mapsLink} target="_blank" className="bg-gray-100 hover:bg-gray-200 text-gray-900 p-4 rounded-2xl flex items-center justify-center transition-colors">
                            ...
                            {submittedData ? (
                                <div className="text-center py-4">
                                    ...
                                    <div className="bg-white/60 rounded-xl p-4 text-left border border-gray-200 text-sm space-y-2">
                                        ...
                                        <div className="flex justify-between"><span className="text-gray-400">Status</span><span className={`font-bold ${submittedData.status === 'hadir' ? 'text-green-600' : submittedData.status === 'ragu' ? 'text-yellow-600' : 'text-red-500'}`}>{submittedData.status === 'hadir' ? 'Will Attend' : submittedData.status === 'ragu' ? 'Maybe' : 'Cannot Attend'}</span></div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-1">
                                            ...
                                            <div className="relative">
                                                <select value={rsvpStatus} onChange={(e) => setRsvpStatus(e.target.value as RsvpStatus)} className="w-full glass-input p-4 rounded-xl appearance-none text-gray-800 font-medium">
                                        ...
                                        {rsvpStatus === 'hadir' && (
                                            <div className="space-y-1">
                                                ...
                                                <div className="relative">
                                                    <select value={rsvpPax} onChange={(e) => setRsvpPax(Number(e.target.value))} className="w-full glass-input p-4 rounded-xl appearance-none text-gray-800 font-medium">
                        ...
                        {banks.map((bank, i) => (
                            <div key={i} className="glass-panel p-6 rounded-3xl shadow-md relative bg-white/80">
                                ...
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <button onClick={() => copyRek(bank.number)} className="p-2 bg-gray-100 rounded-full hover:bg-gray-200 transition"><Copy size={16} /></button>
                                </div>
