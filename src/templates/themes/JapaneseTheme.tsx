...
export default function SakuraTheme({ groom, bride, date, guestName, data }: TemplateProps) {
                                    ...
                                    <div className="bg-[#fff0f3] p-6 rounded-2xl rounded-tl-none border border-pink-100 shadow-sm relative hover:-translate-y-1 transition duration-300">
                                        ...
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <span className="bg-white text-[#db2777] text-[10px] font-bold px-2 py-1 rounded-full shadow-sm border border-pink-100">
                                                {resolveSchedule(data).akadTime}
                                            </span>
                                        </div>

                                        <div className="text-sm text-gray-600 space-y-2">
                                            <p className="flex items-center gap-2"><Calendar size={14} className="text-[#db2777]" /> {formattedAkadDate}</p>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="bg-white p-6 rounded-2xl rounded-tl-none md:rounded-tl-2xl md:rounded-tr-none border border-gray-200 shadow-lg shadow-pink-50 relative hover:-translate-y-1 transition duration-300">
                                        ...
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <span className="bg-[#831843] text-white text-[10px] font-bold px-2 py-1 rounded-full shadow-sm">
                                                {resolveSchedule(data).resepsiTime}
                                            </span>
                                        </div>

                                        <div className="text-sm text-gray-600 space-y-2">
                                            <p className="flex items-center gap-2"><Calendar size={14} className="text-[#831843]" /> {formattedResepsiDate}</p>
                                        </div>
                                    </div>
                                </div>

                                {/* Venue */}
                                <div className="w-full md:w-1/2 md:pl-12 pl-16 md:pr-0 mt-4">
                                    <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-lg relative">
                                        <div className="absolute top-6 -left-2 w-4 h-4 bg-white transform rotate-45 border-l border-b border-gray-200 block md:hidden"></div>
                                        <div className="absolute top-1/2 -left-2 w-4 h-4 bg-white transform -translate-y-1/2 rotate-45 border-l border-b border-gray-200 hidden md:block"></div>

                                        <div className="flex justify-between items-start mb-2">
                                            <h3 className="font-serif-mod text-xl md:text-2xl font-bold text-[#db2777]">Lokasi</h3>
                                            <span className="bg-[#db2777] text-white text-[10px] font-bold px-2 py-1 rounded-full shadow-sm">
                                                <MapPin size={10} />
                                            </span>
                                        </div>
                                        ...
                                        <div className="text-sm text-gray-600 space-y-2">
                                            <p className="flex items-start gap-2">
                                                <MapPin size={14} className="text-[#db2777] mt-1 shrink-0" />
                                                <span className="flex flex-col">
                                                    <span className="font-medium">{resolveVenue(data).name}</span>
                                                    <span className="text-sm opacity-80">{resolveVenue(data).address}</span>
                                                </span>
                                            </p>
                                        </div>

                                        <a href={resolveVenue(data).mapsLink} target="_blank" className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-[#db2777] border-b border-[#db2777] pb-0.5 hover:text-[#831843] transition">
                            ...
                            {banks.map((bank, i) => (
                                <div key={i} className="flex items-center justify-between bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <button onClick={() => copyRek(bank.number)} className="p-2 bg-pink-50 rounded-full hover:bg-pink-100 text-[#db2777] transition">
                                            <Copy size={14} />
                                        </button>
                                    </div>
                                </div>
