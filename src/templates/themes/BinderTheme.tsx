...
import type { BankAccount, RsvpRow, RsvpStatus } from '../../types/database';

import { useCountdown } from '../../hooks/useCountdown';

import { DEFAULT_PHOTOS, formatDate, resolveBanks, resolveGallery, resolveVenue, resolveSchedule, type TimeLeft } from '../../utils/templateHelpers';
...
const PageCover = ({  groom, bride, date, guestName, onNext  }: { groom: string; bride: string; date: string; guestName: string; onNext: () => void })  => (
...
const PageIndex = ({  onJump  }: { onJump: (page: number) => void })  => (
...
const PageCouple = ({  data, groom, bride  }: { data?: TemplateData; groom: string; bride: string })  => (
        ...
        <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="bg-white p-3 shadow-md transform rotate-1 transition hover:rotate-0 hover:z-10 hover:scale-105 duration-300 relative pointer-events-none md:pointer-events-auto">
                ...
                <div className="aspect-[3/4] bg-gray-200 overflow-hidden mb-3">
                    ...
                    <img src={data?.groom_photo || DEFAULT_PHOTOS.groom} className="w-full h-full object-cover grayscale hover:grayscale-0 transition duration-500" alt="Groom" />
            ...
            <div className="bg-white p-3 shadow-md transform -rotate-1 transition hover:rotate-0 hover:z-10 hover:scale-105 duration-300 relative pointer-events-none md:pointer-events-auto">
                ...
                <div className="aspect-[3/4] bg-gray-200 overflow-hidden mb-3">
                    ...
                    <img src={data?.bride_photo || DEFAULT_PHOTOS.bride} className="w-full h-full object-cover grayscale hover:grayscale-0 transition duration-500" alt="Bride" />
...
const PageEvent = ({  data, date  }: { data?: TemplateData; date?: string })  => {
    const schedule = resolveSchedule(data, date);
    const fallbackDate = date ? formatDate(date, 'id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) : '';
        ...
        <div className="space-y-8 font-hand text-[#3E2723]">
            ...
            <div className="flex gap-4 items-start">
                ...
                <div className="flex-1 border-l-2 border-[#8D6E63] pl-4">
                    ...
                    <p className="text-lg">{formattedAkadDate}</p>

                    <p className="text-lg">{schedule.akadTime}</p>
            ...
            <div className="flex gap-4 items-start">
                ...
                <div className="flex-1 border-l-2 border-[#5D4037] pl-4">
                    ...
                    <p className="text-lg">{formattedResepsiDate}</p>

                    <p className="text-lg">{schedule.resepsiTime}</p>
...
const PageLocation = ({  data  }: { data?: TemplateData })  => (
    ...
    <div className="h-full p-8 md:p-12 flex flex-col items-center justify-center bg-[#F5F5DC] select-none">
        ...
        <div className="bg-white p-2 shadow-lg border border-gray-200 transform -rotate-1 w-full max-w-md">
            ...
            <div className="p-4 text-center">
                ...
                <h3 className="font-serif text-2xl text-[#3E2723] mb-1">{resolveVenue(data).name}</h3>

                <p className="font-hand text-sm text-gray-600 mb-4">{resolveVenue(data).address}</p>

                <a href={resolveVenue(data).mapsLink} target="_blank" rel="noreferrer" className="inline-block bg-[#5D4037] text-white px-6 py-2 rounded-full font-bold text-sm hover:bg-[#3E2723] transition shadow-md cursor-pointer pointer-events-auto relative z-20">
...
const PageCountdown = ({  timeLeft  }: { timeLeft: TimeLeft })  => (
...
const PageRsvp = ({  data, submittedData, onRsvpSubmit  }: { data?: TemplateData; submittedData?: RsvpRow | null; onRsvpSubmit?: (payload: RsvpPayload) => Promise<void> })  => {
    ...
    // Stop propagation agar form bisa diklik tanpa trigger swipe (mouse & touch)
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        await onRsvpSubmit?.({ ...formData, status: formData.status as RsvpStatus });
                ...
                {submittedData ? (
                    ...
                    <form onSubmit={handleSubmit} className="space-y-3 font-hand">
                        ...
                        <div className="w-full">
                            ...
                            {/* Indikator sisa karakter (Minimalist style) */}
...
const PageGift = ({  data  }: { data?: TemplateData })  => {
...
const PageGallery = ({  data  }: { data?: TemplateData })  => (
    ...
    <div className="h-full p-6 md:p-12 overflow-y-auto no-scrollbar bg-[#3E2723] select-none">
        ...
        <div className="grid grid-cols-2 gap-4">
            ...
            {resolveGallery(data).map((img: string, i: number) => (
...
const PageClosing = ({  groom, bride, onBack  }: { groom: string; bride: string; onBack: (page: number) => void })  => (
...
export default function BinderBookTheme({
    ...
    const { playing: isAudioPlaying, setPlaying: setIsAudioPlaying, toggle: toggleAudio } = useAudioToggle(audioRef);
    useEffect(() => {
        if (audioRef.current && data?.audio_url) {
            audioRef.current.play().then(() => setIsAudioPlaying(true)).catch(() => setIsAudioPlaying(false));
        }
    }, [data?.audio_url, setIsAudioPlaying]);
    ...
    const jumpToPage = (pageNum: number)  => setPage(pageNum);
    ...
    const onMouseDown = (e: React.MouseEvent)  => { isDragging.current = true; touchStart.current = e.clientX; };

    const onMouseMove = (e: React.MouseEvent)  => { if (!isDragging.current) return; touchEnd.current = e.clientX; };
    ...
    const renderPageContent = ()  => {
        ...
        switch (page) {
            ...
            case 3: return <PageEvent data={data} date={date} />;
    ...
    return (
        ...
        `}</style>
            ...
            <audio ref={audioRef} src={data?.audio_url || "https://r2.loverse.id/defaults/audio/ee2e74c72c.mp3"} loop />
