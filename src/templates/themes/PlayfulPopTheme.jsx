...
export default function PlayfulPastelTheme({ groom, bride, date, guestName, data, onRsvpSubmit, submittedData }) {
            ...
            <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-6 relative z-10">
                <div className="bg-white rounded-2xl overflow-hidden shadow-lg flex flex-col border-2 border-[#BBF7D0] relative group">
                    ...
                    <div className="p-6 text-center space-y-3">
                         <div className="flex items-center justify-center gap-2 text-[#15803D] font-bold bg-[#DCFCE7] py-2 rounded-lg"><Clock size={18}/> {data?.akad_time}</div>
                ...
                <div className="bg-white rounded-2xl overflow-hidden shadow-lg flex flex-col border-2 border-[#FBCFE8] relative group mt-4 md:mt-0">
                    ...
                    <div className="p-6 text-center space-y-3">
                         <div className="flex items-center justify-center gap-2 text-[#9D174D] font-bold bg-[#FCE7F3] py-2 rounded-lg"><Clock size={18}/> {data?.resepsi_time}</div>
                         <p className="font-hand text-lg">{data?.venue_name}</p>
                         <div className="w-full border-t-2 border-dashed border-gray-100 my-4"></div>
                         <a href={data?.maps_link} target="_blank" className="block mt-4 bg-[#9D174D] text-white py-2 rounded-lg font-blob text-sm hover:bg-[#831843] transition">Google Maps 🚀</a>
                    </div>
                    <div className="absolute top-1/2 -left-3 w-6 h-6 bg-[#F0FDF4] rounded-full"></div>
                    <div className="absolute top-1/2 -right-3 w-6 h-6 bg-[#F0FDF4] rounded-full"></div>
                </div>
                    ...
                    {submittedData ? (
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <select value={rsvpStatus} onChange={(e) => setRsvpStatus(e.target.value)} className="input-fun w-full bg-white text-gray-700">
                            ...
                            {rsvpStatus === 'hadir' && (
                                <select value={rsvpPax} onChange={(e) => setRsvpPax(e.target.value)} className="input-fun w-full bg-white text-gray-700">
                                    {[1, 2, 3, 4, 5].map(n => <option key={n} value={n}>{n} Orang</option>)}
                                </select>
                            )}
                            <textarea required value={rsvpMessage} onChange={(e) => setRsvpMessage(e.target.value)} className="input-fun w-full h-32 bg-white" placeholder="Tulis doa yang manis..."></textarea>
                    ...
                    {banks.map((bank, i) => (
                        <div key={i} className="bg-[#EFF6FF] p-4 rounded-2xl border border-blue-100 flex items-center justify-between">
                            ...
                            <div className="flex items-center bg-white px-3 py-1 rounded-lg border border-blue-50">
                                ...
                                <button onClick={() => navigator.clipboard.writeText(bank.number)} className="text-gray-400 hover:text-blue-500"><Copy size={16}/></button>
                            </div>
                        </div>
                    ))}
                </div>
