...
export default function Navbar() {
  ...
  const handleLogout = () => {
    if (window.confirm("Yakin ingin keluar dari Dashboard?")) {
      // Hapus HANYA key session dashboard — jangan sessionStorage.clear()
      // agar tidak menghapus data session milik fitur lain.
    ...
    <nav className="bg-[#712E1E] px-4 md:px-12 py-3 md:py-4 flex justify-between items-center shadow-lg sticky top-0 z-50 transition-all duration-300">
      ...
      <div className="flex gap-2 md:gap-4 items-center">
        ...
        {isLandingPage && (
          <Link
            to="/contact"
            className="px-3 md:px-5 py-2 rounded-full bg-[#F1E8DC] text-[#712E1E] text-xs md:text-base font-medium hover:bg-white transition flex items-center gap-1 md:gap-2 whitespace-nowrap"
          >
