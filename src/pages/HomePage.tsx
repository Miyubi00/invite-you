        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3.5 md:gap-6 lg:gap-7 mx-auto">
            {filteredTemplates.map((template) => (
              <div
                key={template.slug}
                className="bg-white p-3 md:p-4 rounded-2xl shadow-sm border border-[#EBDFCE] flex flex-col items-start transition-all hover:-translate-y-1 hover:shadow-md duration-300 h-full group"
              >
                {/* Gambar Cover */}
                <div className="w-full aspect-[4/3] bg-[#FAF6EE] rounded-xl mb-3 md:mb-4 overflow-hidden relative">
                  <SkeletonImage
                    src={template.image}
                    alt={template.name}
                    width={800}
                  ...
                  <div
                    className={`
                      absolute top-2 left-2 md:top-3 md:left-3 px-2.5 py-0.5 rounded-full text-[9px] md:text-[10px] font-bold uppercase tracking-wider shadow-sm
                ...
                {/* Nama & Harga */}
                <div className="w-full flex flex-col items-start mb-3 md:mb-4 px-0.5">
                  <h3 className="text-[#712E1E] text-sm md:text-base font-bold leading-tight line-clamp-1 text-left">
                    {template.name}
                  </h3>
                  <span className="text-sm md:text-base font-bold text-[#E59A59] mt-0.5">
                ...
                <div className="flex gap-2 w-full mt-auto">
                  <Link
                    ...
                    className="flex-1 py-2 rounded-xl border border-[#E59A59] text-[#E59A59] font-bold text-xs hover:bg-[#FAF6EE] transition flex justify-center items-center gap-1.5"
                  >
                    <Eye className="w-3.5 h-3.5" />
                    <span className="hidden xs:inline">{t('home.catalogPreview')}</span>
                    <span className="inline xs:hidden">{t('home.catalogPreviewShort')}</span>
                  </Link>

                  <Link
                    to={`/order?template=${template.slug}`}
                    className="flex-1 py-2 rounded-xl bg-[#E59A59] text-white font-bold text-xs hover:bg-[#d48b4b] shadow-sm transition flex justify-center items-center gap-1.5"
                  >
